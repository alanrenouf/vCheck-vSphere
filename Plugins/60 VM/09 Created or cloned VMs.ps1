...
Get-VIEventPlus -Start ((get-date).adddays(-$VMsNewRemovedAge)) -EventType @("VmCreatedEvent", "VmBeingClonedEvent", "VmBeingDeployedEvent") | Where-Object { $_.Info.EntityName -in $VM.Name } | Select-Object createdTime, UserName, fullFormattedMessage
